...
def ec_density_F(z, df):
    if z < 0:
        return [
            1,
            np.inf,
        ]  # to bypass warnings in critical threshold calculation
    k, v = map(float, df)
...
def rft(
    c, k, STAT, Z, df, R, n=1, Q=None, expectations_only=False, version="spm12"
):
    """
...
def _approx_threshold(STAT, alpha, df, resels, n):
    ...
    if STAT == "Z":
        ...
        raise (
            ValueError,
            'Statistic must be one of: "Z", "T", "X2", "F", "T2"',
        )
    return zstar
...
def isf(STAT, alpha, df, resels, n, Q=None, version="spm12"):
    ...
    for aaa in alpha:
        ...
        fn = (
            lambda x: (
                rft(1, 0, STAT, x[0], df, resels, n, Q, False, version)[0]
                - aaa
            )
            ** 2
        )
