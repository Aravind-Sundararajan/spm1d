...
from spm1d.stats.anova.factors import (
    Factor,
    FactorNested,
    FactorNested2,
    FactorNestedTwoWay,
)  # FactorRM,FactorSubject
...
class Contrasts(object):
    ...
    def plot(self, ax=None):
        ...
        pyplot.setp(
            ax,
            xticks=range(0, self.C.shape[1], xskip),
            yticks=range(0, self.C.shape[0], yskip),
        )
...
class _Design(object):
    ...
    def plot(self, ax=None, plot_contrasts=True, contrastnums=[0, 1, 2]):
        ...
        pyplot.setp(
            ax0,
            xticks=range(0, self.X.shape[1], xskip),
            yticks=range(0, self.J, yskip),
        )
        ### plot contrasts:
