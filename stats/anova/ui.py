...
from spm1d.stats import _datachecks, _reml, _spm, _spmlist
...
def aov(model, contrasts, f_terms, nFactors=1):
    ...
    for term0, term1 in f_terms:
        ...
        if model.dim == 0:
            F.append(
                _spm.SPM0D_F(
                    f, (df0, df1), (ss0, ss1), (ms0, ms1), model.eij, model.QT
                )
            )
        else:
...
def anova1rm(Y, A, SUBJ, equal_var=True, roi=None, _force_approx0D=False):
    ...
    if not equal_var:
        raise (
            NotImplementedError(
                'Non-sphericity corrections are not yet implemented. Set "equal_var" to "True" to force an assumption of equal variance.'
            )
        )
    design = designs.ANOVA1rm(A, SUBJ)
    model = models.LinearModel(Y, design.X, roi=roi)
    if ((model.dim == 1) or _force_approx0D) and (
        design.check_for_single_responses(dim=model.dim)
    ):
...
def anova2nested(Y, A, B, equal_var=True, roi=None):
    ...
    if equal_var is not True:
        raise (
            NotImplementedError(
                'Non-sphericity correction not implemented. To continue you must assume equal variance and set "equal_var=True".'
            )
        )
...
def anova2onerm(
    Y, A, B, SUBJ, equal_var=True, roi=None, _force_approx0D=False
):
    """
...
def anova3rm(
    Y, A, B, C, SUBJ, equal_var=True, roi=None, _force_approx0D=False
):
    """
