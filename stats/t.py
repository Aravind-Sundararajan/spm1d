...
from spm1d.stats import _datachecks, _reml, _spm
...
def glm(Y, X, c, Q=None, roi=None):
    ...
    ### compute t statistic
    t = np.array(c.T * b).flatten() / (
        np.sqrt(sigma2 * float(c.T * (np.linalg.inv(X.T * X)) * c)) + eps
    )
    ...
    if Y.shape[1] > 1:
        ...
        if roi is None:
            ...
            B = np.logical_and(
                np.logical_not(B), roi
            )  # node is true if in ROI and also not NaN
...
def ttest_paired(YA, YB, roi=None):
    ...
    >>> ti.plot()
    """
    YA, YB = _datachecks.asmatrix(YA, dtype=float), _datachecks.asmatrix(
        YB, dtype=float
    )
