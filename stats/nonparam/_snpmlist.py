...
import numpy as np
from spm1d.stats.nonparam import _snpm
from spm1d.stats import _spmlist
...
class SnPMFiList0D(_spmlist.SPMFiList, _snpm._SnPM):
    ...
    def __init__(self, snpmlist, alpha, zstarvalues, pvalues, iterations):
        ...
        self.permuter = (
            snpmlist.permuter
        )  # permuter (for conducting inference)
        self.nPermUnique = (
            snpmlist.permuter.nPermTotal
        )  # number of unique permutations possible
...
class SnPMFList(_spmlist.SPMFList, _snpm._SnPM1D):
    ...
    def inference(
        ...
        for F, zstar in zip(self, zstarlist):
            clusters = F._get_clusters(
                zstar, False, interp, circular, iterations, cluster_metric
            )
