...
import numpy as np
from spm1d.stats.nonparam.metrics import metric_dict
from spm1d.stats import _spm
from spm1d.stats._spm import p2string, plist2string, _SPMF
from spm1d.stats._clusters import ClusterNonparam
...
class _SnPM0D(_SnPM):
    ...
    def __init__(self, z, perm=None, isinlist=False):
        ...
        self.nPermUnique = (
            None if isinlist else perm.nPermTotal
        )  # number of unique permutations possible
        self.z = z  # test statistic
        self.isinlist = (
            isinlist  # is this object in a list (i.e. two- or three-way ANOVA)
        )
    ...
    def inference(self, alpha=0.05, iterations=-1, force_iterations=False):
        if self.isinlist:
            raise (
                NotImplementedError(
                    "Non-parametric inference must be conducted using the parent SnPMList (for two- and three-way ANOVA)."
                )
            )
...
class SnPM0Dinference(_SnPM0D):
    ...
    def __init__(self, spm, alpha, zstar, p, two_tailed=False, isinlist=False):
        super(SnPM0Dinference, self).__init__(
            spm.z, spm.permuter, isinlist=isinlist
        )
        self.STAT = spm.STAT
        self.PDF = None if isinlist else self.permuter.Z  # permutation PDF
        self.alpha = alpha  # Type I error rate
        self.nPerm = (
            None if isinlist else self.permuter.Z.size
    ...
    def __repr__(self):
        ...
        if not self.isinlist:
            s += (
                "   SnPM.nPermActual   :  %d actual permutations\n"
                % self.nPerm
            )
        s += "   SPM.alpha          :  %.3f\n" % self.alpha
        if self.two_tailed:
...
class SnPM0DiF(SnPM0Dinference, _SPMF):
    ...
    def _repr_summ(self):
        return "{:<5} F = {:<8} p = {}\n".format(
            self.effect.split(" ")[1], "%.3f" % self.z, p2string(self.p)
        )
...
class _SnPM1D(_SnPM, _spm._SPM):
    ...
    def _get_clusters(
        ...
        clusters = super(_SnPM1D, self)._get_clusters(
            zstar, two_tailed, interp, circular, z=z
        )
    ...
    def inference(
        ...
        circular=False,
        force_iterations=False,
        cluster_metric="MaxClusterIntegral",
    ):
        self._check_iterations(iterations, alpha, force_iterations)
        ### build primary PDF:
        self.permuter.build_pdf(iterations)
        ...
        clusters = self._get_clusters(
            zstar, two_tailed, interp, circular, iterations, cluster_metric
        )
...
class _SnPM1Donetailed(_SnPM1D):
    def inference(
        ...
        return super(_SnPM1Donetailed, self).inference(
            alpha=alpha,
...
class SnPM_F(_SnPM1Donetailed, _SPMF):
    ...
    def _repr_summ(self):
        return "{:<5} F = {:<18}\n".format(
            self.effect_short, self._repr_teststat_short()
        )
...
class SnPMiF(SnPMinference, _SPMF):
    ...
    def _repr_summ(self):
        return "{:<5} z={:<18} h0reject={}\n".format(
            self.effect_short, self._repr_teststat_short(), self.h0reject
        )
