...
import numpy as np
from spm1d.stats.ci import CIOneSample0D, CIOneSample1D
from spm1d.stats.ci import (
    CIPairedSample0D,
    CIPairedSample1D,
    CIPairedDifference0D,
    CIPairedDifference1D,
)
from spm1d.stats.ci import (
    CITwoSample0D,
    CITwoSample1D,
    CITwoSampleDifference0D,
    CITwoSampleDifference1D,
)
from spm1d.stats import ttest as ttest_parametric
from spm1d.stats import ttest2 as ttest2_parametric
from spm1d.stats.nonparam.stats import ttest, ttest2
...
def _snpmi2spmi(snpmi):
    snpmi.zstar = (
        snpmi.zstar if np.size(snpmi.zstar) == 1 else max(snpmi.zstar)
    )
    snpmi.df = None
    snpmi.fwhm = None
    snpmi.resels = None
...
def ci_onesample(y, alpha=0.05, mu=None, iterations=-1):
    spmi = ttest(y, mu).inference(
        alpha, two_tailed=True, iterations=iterations
    )
...
def ci_pairedsample(
    yA, yB, alpha=0.05, datum="difference", mu=None, iterations=-1
):
    spmi = ttest(yA - yB, 0).inference(
        alpha, two_tailed=True, iterations=iterations
    )
    spmi = _snpmi2spmi(spmi)
    ...
    if datum == "difference":
        CIClass = (
            CIPairedDifference1DNP if spmi.dim == 1 else CIPairedDifference0DNP
        )
...
def ci_twosample(
    ...
    if datum == "difference":
        CIClass = (
            CITwoSampleDifference1DNP
            if spmi.dim == 1
            else CITwoSampleDifference0DNP
        )
        ci = CIClass(spmi, mA - mB, hstar, mu)
    else:
