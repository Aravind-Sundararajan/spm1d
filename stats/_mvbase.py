...
def _normalize_residuals(R):
    ...
    for i in range(nCurves):
        mag = np.sqrt(
            (R[i, :, :] ** 2).sum(axis=0)
        )  # normalize vector at this time node
        R[i, :, :] /= mag
        mag = np.sqrt(
            (R[i, :, :] ** 2).sum(axis=1)
        )  # normalize the time series energy
...
def _resel_counts(R, W, roi=None):
    ...
    B = np.any(
        np.any(np.abs(R) > 0, axis=0), axis=1
    )  # False indicates no observations at that node
    if roi is not None:
    ...
    rCounts.append(
        (mNodes - mClusters) / float(W)
    )  # number of non-zero nodes, normalized by FWHM
    return rCounts
