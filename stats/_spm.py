...
from spm1d.plot import plot_spm, plot_spm_design
from spm1d.plot import plot_spmi, plot_spmi_p_values, plot_spmi_threshold_label
from spm1d.stats._clusters import Cluster
...
def _set_docstr(childfn, parentfn, args2remove=None):
    ...
    if args2remove != None:
        ...
        for s in docstrlist0:
            if not np.any(
                [s.startswith("- *%s*" % argname) for argname in args2remove]
            ):
...
class SPM0D_F(_SPMF, _SPM0D):
    ...
    def _repr_summ(self):
        return "{:<5} F = {:<8} df = {}\n".format(
            self.effect_short, "%.3f" % self.z, dflist2str(self.df)
        )
...
class SPM0D_T(_SPM0D):
    def __init__(self, z, df, beta=None, residuals=None, sigma2=None):
        _SPM0D.__init__(
            self, "T", z, df, beta=beta, residuals=residuals, sigma2=sigma2
        )
...
class _SPM(_SPMParent):
    ...
    def _build_spmi(self, alpha, zstar, clusters, p_set, two_tailed):
        ...
        if self.STAT == "T":
            spmi = SPMi_T(
                self, alpha, zstar, clusters, p_set, p_clusters, two_tailed
            )
        elif self.STAT == "F":
            spmi = SPMi_F(
                self, alpha, zstar, clusters, p_set, p_clusters, two_tailed
    ...
    def _cluster_geom(self, u, interp, circular, csign=+1, z=None):
        ...
        for i in range(n):
            ...
            if (x[0] > 0) and not np.isnan(
                Z[x[0] - 1]
            ):  # first cluster point not continuum edge && previous point not outside ROI
                z0, z1 = Z[x[0] - 1], Z[x[0]]
                dx = (z1 - u) / (z1 - z0)
                x = [x[0] - dx] + x
                z = [u] + z
            if (x[-1] < Q - 1) and not np.isnan(
                Z[x[-1] + 1]
            ):  # last cluster point not continuum edge && next point not outside ROI
                z0, z1 = Z[x[-1]], Z[x[-1] + 1]
                dx = (z0 - u) / (z0 - z1)
                x += [x[-1] + dx]
                z += [u]
        ...
        if circular and (clusters != []):
            ...
            if (len(ind0) > 0) and (len(ind1) > 0):
                ...
                if (ind0 != ind1) and (
                    clusters[ind0].csign == clusters[ind1].csign
                ):
    ...
    def _get_clusters(self, zstar, check_neg, interp, circular, z=None):
        ...
        if check_neg:
            clustersn = self._cluster_geom(
                zstar, interp, circular, csign=-1, z=z
            )
            clusters += clustersn
            if len(clusters) > 1:
    ...
    def _isf(
        ...
        if self.STAT == "T":
            zstar = rft1d.t.isf_resels(
                a, self.df[1], self.resels, withBonf=withBonf, nNodes=self.Q
            )
        elif self.STAT == "F":
            zstar = rft1d.f.isf_resels(
                a, self.df, self.resels, withBonf=withBonf, nNodes=self.Q
            )
        elif self.STAT == "T2":
            zstar = rft1d.T2.isf_resels(
                a, self.df, self.resels, withBonf=withBonf, nNodes=self.Q
            )
        elif self.STAT == "X2":
            zstar = rft1d.chi2.isf_resels(
                a, self.df[1], self.resels, withBonf=withBonf, nNodes=self.Q
            )
        return zstar
    ...
    def inference(
        ...
        if self.roi is not None:
            if self.roi.dtype != bool:
                if two_tailed:
                    raise (
                        ValueError(
                            "If the ROI contains directional predictions two_tailed must be FALSE."
                        )
                    )
                else:
        ...
        ### conduct inference:
        a = (
            0.5 * alpha if two_tailed else alpha
        )  # adjusted alpha (if two-tailed)
        zstar = self._isf(
            a, withBonf
        )  # critical threshold (RFT inverse survival function)
        clusters = self._get_clusters(
            zstar, check_neg, interp, circular
        )  # assemble all suprathreshold clusters
        clusters = self._cluster_inference(
            clusters, two_tailed, withBonf
        )  # conduct cluster-level inference
        p_set = self._setlevel_inference(
            zstar, clusters, two_tailed, withBonf
        )  # conduct set-level inference
        spmi = self._build_spmi(
            alpha, zstar, clusters, p_set, two_tailed
        )  # assemble SPMi object
        return spmi
...
class SPM_F(_SPMF, _SPM):
    ...
    def _repr_summ(self):
        return "{:<5} z = {:<18} df = {}\n".format(
            self.effect_short, self._repr_teststat_short(), dflist2str(self.df)
        )

    def inference(
        self, alpha=0.05, cluster_size=0, interp=True, circular=False
    ):
        """
...
class SPM_T(_SPM):
    ...
    def inference(
        ...
        """
        return _SPM.inference(
            self, alpha, cluster_size, two_tailed, interp, circular
        )
...
class _SPMinference(_SPM):
    ...
    def __init__(
        self, spm, alpha, zstar, clusters, p_set, p, two_tailed=False
    ):
        _SPM.__init__(
            self,
            spm.STAT,
...
_set_docstr(
    _SPMinference.plot_p_values, plot_spmi_p_values, args2remove=["spmi"]
)
_set_docstr(
    _SPMinference.plot_threshold_label,
