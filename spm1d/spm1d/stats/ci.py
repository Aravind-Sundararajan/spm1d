...
import numpy as np
from spm1d.plot import (
    plot_ci_0d,
    plot_ci_multisample_0d,
    plot_ci,
    plot_ci_multisample,
)
from spm1d.stats._spm import df2str, dflist2str
from spm1d.stats.t import ttest, ttest2
...
class _CI(object):
    ...
    def _get_repr_header(self):
        ...
        s += "   datum         :  %s  %s\n" % (
            self.datum_type,
            self.get_datum_value(asstr=True, with_brackets=True),
        )
        if self.istest:
            s += "   criterion     :  %s  %s\n" % (
                self.criterion_type,
                self.get_criterion_value(asstr=True, with_brackets=True),
            )
        return s
    ...
    def _get_repr_ci(self):
        ...
        if self.istest:
            s += "      h0reject   :  %s (%s)\n" % (
                self.h0reject,
                self.get_h0rejection_decision_reason(),
            )
        return s
...
class _CIMultiMean(_CI):
    ...
    def __init__(self, spmi, meanA, meanB, hstar, criterion="meanB"):
        ...
        if criterion == "meanB":
            ...
            self.ciA = (
                self.meanA - 0.5 * self.hstar,
                self.meanA + 0.5 * self.hstar,
            )
            self.ciB = (
                self.meanB - 0.5 * self.hstar,
                self.meanB + 0.5 * self.hstar,
            )
            if self.dim == 1:
                self.ciA = np.asarray(self.ciA)
        ...
        if criterion == "meanB":
            ...
            if self.dim == 0:
                self.criterion_value = (
                    self.ciB[0] if (self.meanA < self.meanB) else self.ciB[1]
                )
            else:
...
def ci_pairedsample(yA, yB, alpha=0.05, datum="difference", mu=None):
    ...
    if datum == "difference":
        CIClass = (
            CIPairedDifference1D if spmi.dim == 1 else CIPairedDifference0D
        )
...
def ci_twosample(
    yA, yB, alpha=0.05, equal_var=True, datum="difference", mu=None
):
    if equal_var is not True:
        raise NotImplementedError(
            'Two-sample confidence interval calculations are currently implemented only for assumed equal variance. Set "equal_var=True" to force an equal variance assumption.'
        )
    muvalue = 0
    if datum == "difference":
        muvalue = 0 if mu is None else mu
    spmi = ttest2(yA - muvalue, yB, equal_var=True).inference(
    ...
    if datum == "difference":
        CIClass = (
            CITwoSampleDifference1D
            if spmi.dim == 1
            else CITwoSampleDifference0D
        )
        ci = CIClass(spmi, mA - mB, hstar, mu)
    else:
