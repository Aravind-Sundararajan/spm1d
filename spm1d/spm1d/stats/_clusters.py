...
class _Cluster(object):
    ...
    def __repr__(self):
        ...
        if self.P is None:
            ...
            if not self.isparam:
                s += (
                    "   nPermUnique     :  %s unique permutations possible\n"
                    % self.get_nPermUnique_asstr()
                )
                s += (
                    "   nPermActual     :  %d actual permutations\n"
                    % self.nPerm
                )
            s += "   P               :  %.5f\n\n" % self.P
        return s
    ...
    def _assemble(self):
        ...
        if (
            self.extent == 0
        ):  # to reproduce results from previous versions, minimum extent must be one (when not interpolated)
            self.extent = 1
...
class Cluster(_Cluster):
    ...
    def inference(self, STAT, df, fwhm, resels, two_tailed, withBonf, nNodes):
        ...
        if STAT == "T":
            p = rft1d.t.p_cluster_resels(
                k, u, df[1], resels, withBonf=withBonf, nNodes=nNodes
            )
            p = min(1, 2 * p) if two_tailed else p
        elif STAT == "F":
            p = rft1d.f.p_cluster_resels(
                k, u, df, resels, withBonf=withBonf, nNodes=nNodes
            )
        elif STAT == "T2":
            p = rft1d.T2.p_cluster_resels(
                k, u, df, resels, withBonf=withBonf, nNodes=nNodes
            )
        elif STAT == "X2":
            p = rft1d.chi2.p_cluster_resels(
                k, u, df[1], resels, withBonf=withBonf, nNodes=nNodes
            )
        self.P = p
...
class ClusterNonparam(Cluster):
    ...
    def set_metric(self, metric, iterations, nPermUnique, two_tailed):
        ...
        x = metric.get_single_cluster_metric_xz(
            self._X, self._Z, self.threshold, two_tailed
        )
        if self.iswrapped:
            x += metric.get_single_cluster_metric_xz(
                self._other._X, self._other._Z, self.threshold, two_tailed
            )
        self.metric_value = x
