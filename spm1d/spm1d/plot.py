...
import numpy as np
from spm1d._plot import DataPlotter, SPMPlotter, SPMiPlotter, _legend_manual
...
def legend_manual(
    ...
    **kwdargs
    ...
    return _legend_manual(
        ax, colors, labels, linestyles, markerfacecolors, linewidths, **kwdargs
    )
...
def plot_ci_multisample_0d(
    ...
    if ci.criterion_type == "meanB":
        ...
        plotter.plot_errorbar(
            ci.meanA, ci.hstar, color=color, x=x0, hbarw=hbarw
        )
        plotter.plot(
            x1, ci.meanB, "o", color=color_criterion, markersize=markersize
        )
        ...
        plotter.plot_errorbar(
            ci.meanA, 0.5 * ci.hstar, color=color, x=x0, hbarw=hbarw
        )
        plotter.plot_errorbar(
            ci.meanB, 0.5 * ci.hstar, color=color_criterion, x=x1, hbarw=hbarw
        )
        y = (
            (ci.meanB - 0.5 * ci.hstar)
            if (ci.meanB > ci.meanA)
            else (ci.meanB + 0.5 * ci.hstar)
        )
...
def plot_ci_multisample(
    ...
    linecolors = (
        linecolors
        if isinstance(linecolors, (tuple, list))
        else [linecolors] * 2
    )
    facecolors = (
        facecolors
        if isinstance(facecolors, (tuple, list))
        else [facecolors] * 2
    )
    edgecolors = (
        edgecolors
        if isinstance(edgecolors, (tuple, list))
        else [edgecolors] * 2
    )
...
def plot_mean_sd(
    ...
    Y = (
        Y
        if roi is None
        else np.ma.masked_array(
            Y, np.vstack([np.logical_not(roi)] * Y.shape[0])
        )
    )
...
def plot_roi(roi, ax=None, facecolor="0.7", alpha=1, edgecolor="w", ylim=None):
    plotter = DataPlotter(ax)
    plotter.plot_roi(
        roi, ylim=ylim, facecolor=facecolor, edgecolor=edgecolor, alpha=alpha
    )
...
def plot_spmi_p_values(
    spmi,
    ax=None,
    size=8,
    offsets=None,
    offset_all_clusters=None,
    autoset_ylim=True,
):
    """
...
def plot_spmi_threshold_label(
    spmi, ax=None, lower=False, pos=None, autoset_ylim=True, **kwdargs
):
    """
    Plot an **spm1d** SPM inference object as a line.
