...
class Besier2009kneeflexion(_base.DatasetANOVA2, _base.Dataset1D):
    def _set_values(self):
        ...
        self.datafile = os.path.join(
            _base.get_datafilepath(), "Besier2009kneeflexion.npz"
        )
        self.note = (
            "Results  ",
            "Pataky, T. C., Vanrenterghem, J., & Robinson, M. A. (2015). Two-way ANOVA for scalar trajectories, with experimental evidence of non-phasic interactions. Journal of Biomechanics, 48(1), 186–189. http://doi.org/10.1016/j.jbiomech.2014.10.013",
        )
        ### load and parse data:
...
class SPM1D_ANOVA2_2x2(_SPM1D_ANOVA2_DATASET):
    def _set_datafile(self):
        self.datafile = os.path.join(
            _base.get_datafilepath(), "spm1d_anova2_2x2.npz"
        )
...
class SPM1D_ANOVA2_2x3(_SPM1D_ANOVA2_DATASET):
    def _set_datafile(self):
        self.datafile = os.path.join(
            _base.get_datafilepath(), "spm1d_anova2_2x3.npz"
        )
...
class SPM1D_ANOVA2_3x3(_SPM1D_ANOVA2_DATASET):
    def _set_datafile(self):
        self.datafile = os.path.join(
            _base.get_datafilepath(), "spm1d_anova2_3x3.npz"
        )
...
class SPM1D_ANOVA2_3x4(_SPM1D_ANOVA2_DATASET):
    def _set_datafile(self):
        self.datafile = os.path.join(
            _base.get_datafilepath(), "spm1d_anova2_3x4.npz"
        )
...
class SPM1D_ANOVA2_3x5(_SPM1D_ANOVA2_DATASET):
    def _set_datafile(self):
        self.datafile = os.path.join(
            _base.get_datafilepath(), "spm1d_anova2_3x5.npz"
        )
...
class SPM1D_ANOVA2_4x4(_SPM1D_ANOVA2_DATASET):
    def _set_datafile(self):
        self.datafile = os.path.join(
            _base.get_datafilepath(), "spm1d_anova2_4x4.npz"
        )
...
class SPM1D_ANOVA2_4x5(_SPM1D_ANOVA2_DATASET):
    def _set_datafile(self):
        self.datafile = os.path.join(
            _base.get_datafilepath(), "spm1d_anova2_4x5.npz"
        )
