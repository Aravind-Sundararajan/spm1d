...
eps = np.finfo(
    float
).eps  # smallest float, used to avoid divide-by-zero errors
...
class DataPlotter(object):
    ...
    def plot_cloud(
        ...
        patches = PatchCollection(
            [Polygon(np.array([x, y]).T)], edgecolors=None
        )
        ### plot:
    ...
    def plot_errorbar(
        ...
        w = hbarw * e
        self.ax.plot(
            [x - w, x + w], [y - e] * 2, "-", color=color, lw=linewidth
        )
        self.ax.plot(
            [x - w, x + w], [y + e] * 2, "-", color=color, lw=linewidth
        )
    ...
    def plot_roi(
        ...
        for i in range(n):
            ...
            pyplot.setp(
                poly, facecolor=facecolor, edgecolor=edgecolor, alpha=alpha
            )
...
class SPMPlotter(DataPlotter):
    ...
    def plot_design(self, factor_labels=None, fontsize=10):
        ...
        if factor_labels != None:
            gs = X.shape
            tx = [
                self.ax.text(i, -0.05 * gs[0], label)
                for i, label in enumerate(factor_labels)
            ]
            pyplot.setp(
                tx, ha="center", va="bottom", color="k", fontsize=fontsize
            )
        self.ax.axis("normal")
        self.ax.axis("off")
...
class SPMiPlotter(SPMPlotter):
    ...
    def plot(
        ...
        self.plot_field(
            color=color, lw=lw, linestyle=linestyle, marker=marker, label=label
        )
        self.plot_datum()
    ...
    def plot_threshold(self, color="k"):
        ...
        if spmi.roi is None:
            ...
            if spmi.roi.dtype == bool:
                zz = np.ma.masked_array(
                    [zs] * spmi.Q, np.logical_not(spmi.roi)
                )
                h = [ax.plot(self.x, zz)]
                if spmi.two_tailed:
                ...
                if np.any(spmi.roi > 0):
                    zz = np.ma.masked_array(
                        [zs] * spmi.Q, np.logical_not(spmi.roi > 0)
                    )
                    h.append(ax.plot(self.x, zz))
                if np.any(spmi.roi < 0):
                    zz = np.ma.masked_array(
...
def _legend_manual(
    ...
    **kwdargs
    ...
    h = [
        ...
        for color, ls, lw, mfc in zip(
            colors, linestyles, linewidths, markerfacecolors
        )
    ]
    ax.set_xlim(x0, x1)
    ax.set_ylim(y0, y1)
...
def _plot_F_list(
    FF, plot_threshold_label=True, plot_p_values=True, autoset_ylim=True
):
    m = FF.nFactors
